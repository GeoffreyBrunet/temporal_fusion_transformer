...
class Process_data:
    ...
    def sort_data(self, path: str) -> pd.DataFrame:
    ...
    def create_new_features(self, df, earliest_time) -> tuple[pd.DataFrame, float]:
        for label in df:
            ts = df[label]
        ...
        time_df = pd.concat(self.df_list).reset_index(drop=True)
    ...
    def generate_data(self, path: str) -> pd.DataFrame:
        df = self.sort_data(path)
        df, earliest_time = self.aggregate_hourly_data(df)
        df = self.create_new_features(df, earliest_time)
        return df
